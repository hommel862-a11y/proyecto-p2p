...
export type QuickOverlayIpcMessage =
  ...
  | {
      channel: typeof QUICK_OVERLAY_IPC_CHANNELS.POSITION_CHANGED;
      payload: { x: number; y: number };
    }
  | {
      channel: typeof QUICK_OVERLAY_IPC_CHANNELS.CONFIG_CHANGED;
      payload: Partial<QuickOverlayConfig>;
    }
 ...
 */
export function validateQuickOverlayConfig(config: Partial<QuickOverlayConfig>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export function createQuickOverlayController() {
  ...
  return {
    ...
    ipc: {
      sendCalculate: (_input: SpreadCalculationInput) => {
        // window.electron.ipcRenderer.invoke(QUICK_OVERLAY_IPC_CHANNELS.CALCULATE, input)
      },
      onResult: (_callback: (result: SpreadCalculationResult) => void) => {
